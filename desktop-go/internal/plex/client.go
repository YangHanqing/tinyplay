...
func (c *Client) Resume(limit int) ([]byte, error) {
	d, e := c.get("/library/onDeck", url.Values{"X-Plex-Container-Size": {strconv.Itoa(limit)}})
	if e != nil {
		return nil, e
	}
	ms := metadata(d)
...
func (c *Client) ChoosePlayURL(id string) (PlayURLChoice, error) {
	d, e := c.get("/library/metadata/"+url.PathEscape(id), nil)
	if e != nil {
