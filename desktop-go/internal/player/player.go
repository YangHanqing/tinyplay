...
type Player struct {
	...
	StopReporter     func(itemID, sessionID string, posTicks int64, mediaSourceID string)
	ProgressReporter func(itemID, sessionID string, posTicks int64, isPaused bool, mediaSourceID string)
	...
	ScreensaverImageProvider func(itemID, posterItemID string, index int) []byte
...
func (p *Player) progressRun() {
	for {
		...
		p.propsMu.Unlock()
		reporter(itemID, p.playSessionID, pos, isPaused, p.mediaSourceID)
	}
}
...
func (p *Player) fireStopReport() {
	...
	go reporter(itemID, p.playSessionID, atomic.LoadInt64(&p.lastPosTicks), p.mediaSourceID)
}
