...
type Media interface {
	...
	Resume(start, limit int) ([]byte, error)
	...
	ChoosePlayURL(id, sourceID string) (PlayChoice, error)
...
func (m *embyMedia) Resume(a, b int) ([]byte, error)        { return m.c.Resume(a, b) }
...
func (m *embyMedia) ChoosePlayURL(a, b string) (PlayChoice, error) {
	x, e := m.c.ChoosePlayURL(a, b)
...
func (m *plexMedia) Resume(a, b int) ([]byte, error)        { return m.c.Resume(a, b) }
...
func (m *plexMedia) ChoosePlayURL(a, b string) (PlayChoice, error) {
	x, e := m.c.ChoosePlayURL(a, b)
	return PlayChoice{x.URL, x.MediaSourceID}, e
}
...
func Active() (Media, error) {
	...
	return FromServerConfig(s)
}

func FromServer(id string) (Media, error) { return FromServerConfig(config.GetServer(id)) }

func FromServerConfig(s *config.Server) (Media, error) {
	if s == nil {
		return nil, &APIError{400, "No media source is available. Add one first."}
	}
	kind := config.NormalizeServerType(s.Type)
	switch {
	case kind == "plex":
	...
	default:
		return &embyMedia{emby.New(s), kind}, nil
	}
}
...
func FileFromServer(id string) (*filesource.Client, error) { return filesource.FromServer(id) }
