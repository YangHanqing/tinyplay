...
func (p *Player) updateScreensaver() {
	...
	if needsImage {
		p.refreshScreensaverBackdrop(ctx.ServerID, ctx.ItemID, ctx.PosterItemID, screenW, screenH)
	}
	p.refreshScreensaverText(ctx, screenW, screenH, now)
}

func (p *Player) refreshScreensaverBackdrop(serverID, itemID, posterItemID string, screenW, screenH int) {
	p.screensaverMu.Lock()
	...
	if p.ScreensaverImageProvider != nil {
		data = p.ScreensaverImageProvider(serverID, itemID, posterItemID, imageIndex)
		if data == nil && imageIndex != 0 {
			data = p.ScreensaverImageProvider(serverID, itemID, posterItemID, 0)
		}
	}
